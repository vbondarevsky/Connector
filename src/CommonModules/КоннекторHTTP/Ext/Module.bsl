...
Функция ПодготовитьЗапрос(Сессия, Метод, URL, ДополнительныеПараметры) Экспорт
	...
	ПараметрыПреобразованияJSON =
		ВыбратьЗначение(Неопределено, ДополнительныеПараметры, "ПараметрыПреобразованияJSON", Неопределено);
	ПараметрыURLИзДополнительныхПараметров =
		ВыбратьЗначение(Неопределено, ДополнительныеПараметры, "ПараметрыURL", Новый Структура);
	...
	ПодготовленныйЗапрос.Вставить("ПараметрыURL", ПараметрыURLИзДополнительныхПараметров);
	ПодготовленныйЗапрос.Вставить("URL", ПодготовитьURL(URL, ПараметрыЗапроса, ПараметрыURLИзДополнительныхПараметров));
...
Функция ПодготовитьURL(Знач URL, ПараметрыЗапроса = Неопределено, ПараметрыURL = Неопределено)

	URL = ЗаполнитьПараметрыURL(URL, ПараметрыURL);
...
// Заменяет в URL имена параметров на их значения. Параметры в строке выделяются с двух сторон фигурными
// скобками.
//
// Параметры:
//  URL             - Строка    - строка, в которую необходимо вставить значения.
//  ПараметрыURL    - Структура - подставляемые значения параметров, где ключ - имя параметра без спецсимволов,
//                             значение - вставляемое значение.
//
// Возвращаемое значение:
//  Строка - URL со вставленными значениями.
// 
// Пример:
//  ПараметрыURL = Новый Структура("id", "123");
//  Результат = ЗаполнитьПараметрыURL("https://developer.site.com/alpha/api/nodes/{id}/versions", ПараметрыURL);
//  - возвращает: "https://developer.site.com/alpha/api/nodes/123/versions".
Функция ЗаполнитьПараметрыURL(URL, ПараметрыURL) //extension
		
	Результат = URL;
	Если ТипЗнч(ПараметрыURL) = Тип("Структура") Тогда
