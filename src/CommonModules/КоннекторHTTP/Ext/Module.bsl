...
//  ДополнительныеПараметры - Структура - см. описание в методе ВызватьМетод
// Возвращаемое значение:
//  Соответствие, Структура - ответ, преобразованный из JSON. 
...
// Параметры:
//  URL - Строка - URL ресурса, к которому будет отправлен запрос.
//  Json - Структура, Соответствие - данные, которые будут преобразованы в JSON и отправленны на сервер.
//  ДополнительныеПараметры - Структура - см. описание в методе ВызватьМетод
// Возвращаемое значение:
//  Соответствие, Структура - ответ, преобразованный из JSON. 
//                            Параметры преобразования см. ДополнительныеПараметры.ПараметрыПреобразованияJSON 
...
Функция КакJson(Ответ, ПараметрыПреобразования = Неопределено) Экспорт
	...
	Возврат JsonВОбъект(Ответ.Тело, Ответ.Кодировка, ПараметрыПреобразования);
...
Функция СоздатьСессию() Экспорт
	...
	Сессия.Вставить("ПулСоединений", Новый Соответствие); // TODO: поддержка Keep-Alive
