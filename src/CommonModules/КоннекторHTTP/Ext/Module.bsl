...
Функция ОписанияКодовСостоянийHTTP()
	...
	КодыСостояния.Добавить(
		НовыйКодСостоянияHTTP(407, "НеобходимаАутентификацияПрокси_407", "Proxy Authentication Required"));
	...
	КодыСостояния.Добавить(
		НовыйКодСостоянияHTTP(431, "ПоляЗаголовкаЗапросаСлишкомБольшие_431", "Request Header Fields Too Large"));
	КодыСостояния.Добавить(НовыйКодСостоянияHTTP(449, "ПовторитьС_449", "Retry With"));
	КодыСостояния.Добавить(
		НовыйКодСостоянияHTTP(451, "НедоступноПоЮридическимПричинам_451", "Unavailable For Legal Reasons"));
	...
	КодыСостояния.Добавить(
		НовыйКодСостоянияHTTP(511, "ТребуетсяСетеваяАутентификация_511", "Network Authentication Required"));
