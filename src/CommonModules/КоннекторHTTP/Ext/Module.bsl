...
Функция ОтправитьЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
	...
	ПодготовленныйОтвет.Вставить("Cookies", ИзвлечьCookies(Ответ.Заголовки, ПодготовленныйЗапрос.URL));  // TODO: Извлечь cookies 
	...
	Для Каждого ОчереднойЗаголовок Из Заголовки Цикл
		Если НРег(ОчереднойЗаголовок.Ключ) = "set-cookie" Тогда
			Для Каждого ЗаголовокCookie Из РазбитьНаОтдельныеЗаголовкиCookies(ОчереднойЗаголовок.Значение) Цикл
				...
				Если Cookies.Получить(Cookie.Домен) = Неопределено Тогда
					Cookies[Cookie.Домен] = Новый Соответствие;
				КонецЕсли;
				Если Cookies[Cookie.Домен].Получить(Cookie.Путь) = Неопределено Тогда
					Cookies[Cookie.Домен][Cookie.Путь] = Новый Соответствие;
				КонецЕсли;
				Cookies[Cookie.Домен][Cookie.Путь][Cookie.Наименование] = Cookie;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;	
...
Функция РаспарситьCookie(Заголовок, URL)
	...
	Cookie = Новый Структура("Наименование, Значение, Домен, Порт, Путь, СрокДействия, ТолькоБезопасноеСоединение");
	Индекс = 0;
