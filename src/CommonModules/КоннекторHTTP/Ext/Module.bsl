...
//  ПараметрыЗапроса - Структура, Соответствие - параметры, которые будут отправлены в параметрах URL:
...
//     **<Значение> - Строка - значение параметра URL
//                  - Массив - сформирует строку из нескольких параметров: key=value1&key=value2 и т.д.                         
//   *Данные - Структура, Соответствие - будут закодированы и отправлены как application/x-www-form-urlencoded.
//           - Строка, ДвоичныеДанные, Поток - данные буду отправлены без изменения в теле запроса.
//   *Json - Структура, Соответствие - данные будут отправлены как строка в формате JSON.
//   *Заголовки - Соответствие - заголовки, которые будут отправлены на сервер в виде соответствия: "Заголовок": "Значение". 
//   *Файлы - Строка
//          - ДвоичныеДанные
//          - Поток
//          - Массив[Строка, ДвоичныеДанные, Поток]
//   *Аутентификация - 
//     * Тип - Строка - 
//     Basic
//     * Пользователь - Строка
//     * Пароль - Строка - 
//     * ИспользоватьАутентификациюОС - Булево - включает использование аутентификации NTLM или Negotiate на сервере.
//   *Cookies - 
...
//   *Прокси - ИнтернетПрокси - прокси, через который нужно установить соединение.
//   *ПроверятьSSL - Булево - 
...
//   *Заголовки - Соответствие - заголовки HTTP, которые будут отправлены на сервер в виде соответствия: "Заголовок": "Значение".
// 
// Возвращаемое значение:
//   - 
//
...
// 	Структура - ответ сервера. 
//
Функция GetJson(URL, ПараметрыЗапроса = Неопределено, ПрочитатьВСоответствие = Истина, ДополнительныеПараметры = Неопределено) Экспорт
...
Функция Post(URL, Данные = Неопределено, Json = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Возврат ВызватьHTTPМетод(
		Неопределено, "POST", URL, Данные, Json, Неопределено, ДополнительныеПараметры);
...
// TODO: Подготовить описание
Функция КакJson(Ответ, ПрочитатьВСоответствие = Истина) Экспорт
	
	Возврат JsonВОбъект(Ответ.Тело, Ответ.Кодировка, ПрочитатьВСоответствие);
...
Функция КакТекст(Ответ, Кодировка = Неопределено) Экспорт
	...
	ЧтениеТекста = Новый ЧтениеТекста(Ответ.Тело.ОткрытьПотокДляЧтения(), Кодировка);
...
Функция СоздатьСеанс() Экспорт
	...
	Сеанс = Новый Структура;
	Сеанс.Вставить("Заголовки", ЗаголовкиПоУмолчанию());
	Сеанс.Вставить("Аутентификация", Неопределено);
	Сеанс.Вставить("Прокси", Неопределено);
	Сеанс.Вставить("ПараметрыЗапроса", Новый Структура);
	Сеанс.Вставить("ПроверятьSSL", Истина);
	Сеанс.Вставить("КлиентскийСертификатSSL", Неопределено);
	Сеанс.Вставить("МаксимальноеКоличествоПеренаправлений", МаксимальноеКоличествоПеренаправлений());
	Сеанс.Вставить("Cookies", Новый Соответствие); // TODO: см. cookiejar_from_dict({})
	
	Сеанс.Вставить("ПулСоединений", Новый Соответствие); // TODO: поддержка Keep-Alive
	
	Возврат Сеанс;
	
КонецФункции

Функция ВызватьМетодВСеансе(Сеанс, Метод, URL, ДополнительныеПараметры = Неопределено) Экспорт
	
	Возврат ВызватьHTTPМетод(Сеанс, Метод, URL, Неопределено, Неопределено, Неопределено, ДополнительныеПараметры);	
...
Функция СкопироватьПодготовленныйЗапрос(Запрос)
	
	НовыйЗапрос = Новый Структура;
	
	// TODO: пока будем хитрить и подсовывать тот запрос
	
	Возврат Запрос;
	
КонецФункции

Функция ВызватьHTTPМетод(Сеанс, Метод, URL, Данные, Json, ПараметрыЗапроса, ДополнительныеПараметры)
	...
	ПодготовленныйЗапрос = ПодготовитьЗапрос(Сеанс, Запрос);
	...
	Ответ = ОтправитьЗапрос(Сеанс, ПодготовленныйЗапрос, Настройки);
	
	
	Перенаправление = 0;
	Пока Перенаправление < МаксимальноеКоличествоПеренаправлений() Цикл
		Если Не Настройки.РазрешитьПеренаправление ИЛИ Не Ответ.ЭтоРедирект Тогда
			Возврат Ответ;
		КонецЕсли;
		
		// TODO: подготовить редирект
		...
		ПодготовленныйЗапрос = СкопироватьПодготовленныйЗапрос(ПодготовленныйЗапрос);
		...
		Если Ответ.КодСостояния <> 307 И Ответ.КодСостояния <> 308 Тогда
			...
			Для Каждого ЗаголовокДляУдаления Из ЗаголовкиДляУдаления Цикл
				ПодготовленныйЗапрос.Заголовки.Удалить(ЗаголовкиДляУдаления);
			КонецЦикла;
		    ПодготовленныйЗапрос.HTTPЗапрос.Заголовки = ПодготовленныйЗапрос.Заголовки;
		КонецЕсли;
		
		// TODO: Делаем что-то с куками, нужно понять точно что должно быть
		
		// TODO: Ребилд прокси
		
		// TODO: Ребилд аутентификацию
		
		
		Ответ = ОтправитьЗапрос(Сеанс, ПодготовленныйЗапрос, Настройки);

		
		// TODO: завершение подготовки редиректа
		
		Перенаправление = Перенаправление + 1;
	КонецЦикла;
	
	ВызватьИсключение("СлишкомМногоПеренаправлений"); // TODO: может что-то другое
		
КонецФункции

Функция ПодготовитьЗапрос(Сеанс, Запрос)
	
	// TODO: с куками разобраться
	// ХранилищеCookies = СоздатьХранилищеCookiesИзСоответствия(Запрос.Cookies);
	
	ПодготовленныйЗапрос = Новый Структура;
	ПодготовленныйЗапрос.Вставить("Cookies", ОбъединитьCookies(Сеанс.Cookies, Запрос.Cookies));
	ПодготовленныйЗапрос.Вставить("Аутентификация", ОбъединитьПараметрыАутентификации(Сеанс.Аутентификация, Запрос.Аутентификация));
	ПодготовленныйЗапрос.Вставить("Метод", Запрос.Метод);
	ПодготовленныйЗапрос.Вставить("Заголовки", ОбъединитьЗаголовки(Запрос.Заголовки, Сеанс.Заголовки));
	ПараметрыЗапроса = ОбъединитьПараметрыЗапроса(Сеанс.ПараметрыЗапроса, Запрос.ПараметрыЗапроса);
...
Процедура ПодготовитьCookies(ПодготовленныйЗапрос)
	...
	//ЗаголовокCookie = ПодготовитьЗаголовокCookie();
	//Если ЗначениеЗаполнено(ПодготовленныйЗапрос.Cookies) Тогда
...
Функция СоздатьПолеФормы(ИсходныеПараметры)
	...
	Если Поле.Свойство("Заголовки") И ЗначениеЗаполнено(Поле.Заголовки) Тогда
		Заголовки = Поле.Заголовки;
		Если ПолучитьЗначениеЗаголовка("content-disposition", Заголовки) = Ложь Тогда
			Заголовки.Вставить("Content-Disposition", "form-data");	
		КонецЕсли;
	Иначе
...
Функция ЗакодироватьФайлы(HTTPЗапрос, Файлы, Данные)
	...
	Если ЗначениеЗаполнено(Данные) Тогда
		Части.Добавить(СоздатьПолеФормы(Данные));
	КонецЕсли;
...
Процедура ПодготовитьТелоЗапроса(ПодготовленныйЗапрос, Данные, Файлы, Json)
	...
	Если Не ЗначениеЗаполнено(Данные) И ЗначениеЗаполнено(Json) Тогда
...
Функция ОтправитьЗапрос(Сеанс, ПодготовленныйЗапрос, Настройки)
	...
	ПодготовленныйОтвет.Вставить("Cookies", ИзвлечьCookies(Ответ.Заголовки));  // TODO: Извлечь cookies 
	...
	// TODO: Выделить в функцию отдельную???
	Если ПолучитьЗначениеЗаголовка("content-encoding", Ответ.Заголовки) = "gzip" Тогда
		ПодготовленныйОтвет.Вставить("Тело", ПрочитатьGZip(ПодготовленныйОтвет.Тело));
	КонецЕсли;
	ПодготовленныйОтвет.Вставить("Запрос", ПодготовленныйЗапрос); // TODO: Может быть только в DEBUG режиме?
...
Функция ИзвлечьCookies(Заголовки)
	...
	Для Каждого ОчереднойЗаголовок Из Заголовки Цикл
		Если НРег(ОчереднойЗаголовок.Ключ) = "set-cookie" Тогда
			Для Каждого ЗаголовокCookie Из РазбитьНаОтдельныеЗаголовкиCookies(ОчереднойЗаголовок.Значение) Цикл
				Cookies.Добавить(РаспарситьCookie(ЗаголовокCookie));
			КонецЦикла;
			Прервать;
		КонецЕсли;
	КонецЦикла;	
...
Функция РаспарситьCookie(Заголовок)
	
	Cookie = Новый Структура("Наименование, Значение, Домен, Путь, СрокДействия, ТолькоБезопасноеСоединение");
	Индекс = 0;
	...
	Для Каждого Параметр Из СтрРазделить(Заголовок, ";", Ложь) Цикл
		...
		ИначеЕсли Ключ = "secure" Тогда
			Cookie.Путь = Истина;
		Иначе
			Продолжить; // TODO: обрабатывать ли другие параметры??
		КонецЕсли; 
	КонецЦикла;
...
Функция ПолучитьСоединение(ПараметрыСоединения, ДополнительныеПараметры)
	...
	Возврат Новый HTTPСоединение(
		...
		"", "",  // TODO: Аутентификация
		ДополнительныеПараметры.Прокси, // TODO: Прокси 
...
Функция JsonВОбъект(Json, Кодировка = "utf-8", ПрочитатьВСоответствие = Истина)
	...
	КонецЕсли;
	Объект = ПрочитатьJSON(ЧтениеJSON, ПрочитатьВСоответствие);
	ЧтениеJSON.Закрыть();
...
Функция ПрочитатьDeflate(СжатыеДанные) // TODO: Или zlib deflate?
