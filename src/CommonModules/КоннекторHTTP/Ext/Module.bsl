...
Функция ПолучитьПроксиПоУмолчанию(URL)
	...
	ИмяОМПолученияФайловБСП = "ПолучениеФайловИзИнтернетаКлиентСервер";
	Если Метаданные.ОбщиеМодули.Найти(ИмяМодуляПолученияФайловБСП) <> Неопределено Тогда
		СтруктураURL = РазобратьURL(URL);
		Модуль = Вычислить(ИмяМодуляПолученияФайловБСП);
