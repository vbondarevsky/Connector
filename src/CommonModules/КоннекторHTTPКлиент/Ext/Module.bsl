...
// Адаптация библиотеки "Коннектор" для работы в клиентском контексте
//
// 2020 petypen@yandex.ru
...
//
&НаКлиенте
Функция ОбъектВJson(Данные, Знач ПараметрыПреобразования = Неопределено, Знач ПараметрыЗаписи = Неопределено) Экспорт
...
Функция ВызватьHTTPМетод(Сессия, Метод, URL, ДополнительныеПараметры)
	...
	Пока Перенаправление < Сессия.МаксимальноеКоличествоПеренаправлений Цикл
		...
		ПодготовленныйЗапрос.URL = КоннекторHTTPВызовСервера.КодироватьСтрокуСервер(НовыйURL, "URLВКодировкеURL");
...
Функция СформироватьНовыйURLПриПеренаправлении(Ответ)
	...
	НовыйURL = КоннекторHTTPВызовСервера.РаскодироватьСтрокуСервер(НовыйURL, "URLВКодировкеURL");
...
Процедура УдалитьЗаголовки(Заголовки, СписокЗаголовковСтрокой)
	...
	Для Каждого ЗаголовокHTTP Из Заголовки Цикл
		Если СписокЗаголовков.Найти(НРег(ЗаголовокHTTP.Ключ)) <> Неопределено Тогда
			ЗаголовкиДляУдаления.Добавить(ЗаголовокHTTP.Ключ);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Параметр Из ПараметрыЗапроса Цикл
		...
		Иначе
			Для Каждого Значение Из Значения Цикл
				ЗначениеПараметра = КоннекторHTTPВызовСервера.КодироватьСтрокуСервер(Значение, "КодировкаURL");
...
Функция ЗаголовкиВСтроку(Заголовки)
	...
	Для Каждого ЗаголовокHTTP Из Заголовки Цикл
		Если Ключи.Найти(ВРег(ЗаголовокHTTP.Ключ)) = Неопределено Тогда
			Строки.Добавить(СтрШаблон("%1: %2", ЗаголовокHTTP.Ключ, ЗаголовокHTTP.Значение));
		КонецЕсли;
	КонецЦикла;
...
Функция ОбъединитьЗаголовки(ГлавныйИсточник, ДополнительныйИсточник)
	...
	Для Каждого ЗаголовокHTTP Из ГлавныйИсточник Цикл
		Заголовки.Вставить(ЗаголовокHTTP.Ключ, ЗаголовокHTTP.Значение);
	КонецЦикла;
	Для Каждого ЗаголовокHTTP Из ДополнительныйИсточник Цикл
		Если Заголовки.Получить(ЗаголовокHTTP.Ключ) = Неопределено Тогда
			Заголовки.Вставить(ЗаголовокHTTP.Ключ, ЗаголовокHTTP.Значение);
		КонецЕсли;
	КонецЦикла;
	...
	Если ЗаголовокRetryAfter <> Ложь Тогда
		...
		Если Длительность = 0 Тогда
			...
			Если ЗначениеЗаполнено(Дата) Тогда
				Длительность = Дата - КоннекторHTTPВызовСервера.ТекущаяУниверсальнаяДатаСервер();
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Функция ИзвлечьCookies(Заголовки, URL)
	...
	ТекущееВремя = КоннекторHTTPВызовСервера.ТекущаяУниверсальнаяДатаСервер();
	Cookies = Новый Соответствие;
...
&НаКлиенте
Функция РазбитьНаОтдельныеЗаголовкиCookies(Знач ЗаголовокHTTP)
	
	Заголовки = Новый Массив;
	
	Если Не ЗначениеЗаполнено(ЗаголовокHTTP) Тогда
		Возврат Заголовки;
	КонецЕсли;
	
	ЗапчастиЗаголовков = СтрРазделить(ЗаголовокHTTP, ",", Ложь);
...
&НаКлиенте
Функция РаспарситьCookie(ЗаголовокHTTP, URL, ТекущееВремя)
	
	Cookie = Неопределено;
	Индекс = 0;
	
	Для Каждого Параметр Из СтрРазделить(ЗаголовокHTTP, ";", Ложь) Цикл
		Индекс = Индекс + 1;
...
&НаКлиенте
Функция ПолучитьЗначениеЗаголовка(ЗаголовокHTTP, ВсеЗаголовки, Ключ = Неопределено)
...
&НаКлиенте
Функция ПолучитьКодировкуИзЗаголовка(Знач ЗаголовокHTTP)

	Кодировка = Неопределено;
	
	ЗаголовокHTTP = НРег(СокрЛП(ЗаголовокHTTP));
	ИндексРазделителя = СтрНайти(ЗаголовокHTTP, ";");
	Если ИндексРазделителя Тогда
		ТипСодержимого = СокрЛП(Лев(ЗаголовокHTTP, ИндексРазделителя - 1));
		КлючКодировки = "charset=";
		ИндексКодировки = СтрНайти(ЗаголовокHTTP, КлючКодировки);
		Если ИндексКодировки Тогда
			ИндексРазделителя = СтрНайти(ЗаголовокHTTP, ";", НаправлениеПоиска.СНачала, ИндексКодировки);
			НачальнаяПозиция = ИндексКодировки + СтрДлина(КлючКодировки);
			Если ИндексРазделителя Тогда
				ДлинаКодировки = ИндексРазделителя - НачальнаяПозиция;
			Иначе
				ДлинаКодировки = СтрДлина(ЗаголовокHTTP);
			КонецЕсли;
			Кодировка = Сред(ЗаголовокHTTP, НачальнаяПозиция, ДлинаКодировки);
			Кодировка = СтрЗаменить(Кодировка, """", "");
			Кодировка = СтрЗаменить(Кодировка, "'", "");
		КонецЕсли;
	Иначе
		ТипСодержимого = ЗаголовокHTTP;
	КонецЕсли;
...
Функция ПолучитьИдентификаторСоединения(ПараметрыСоединения)
	...
	Возврат КоннекторHTTPВызовСервера.ХешированиеДанных("MD5", СтрСоединить(ПараметрыДляРасчетаИдентификатора, ""));
...
Функция ЗаполнитьПараметрыЗапроса(Путь)
	...
	Для Каждого СтрокаКлючРавноПараметр Из СтрРазделить(Запрос, "&", Ложь) Цикл
		СтрокаКлючРавноПараметр = КоннекторHTTPВызовСервера.РаскодироватьСтрокуСервер(СтрокаКлючРавноПараметр, "URLВКодировкеURL");
...
Процедура ПодготовитьАутентификациюAWS4(ПодготовленныйЗапрос)
	...
	ЗначениеЗаголовка = КоннекторHTTPВызовСервера.ПолучитьЗначениеЗаголовкаСервер("x-amz-date", ПодготовленныйЗапрос.Заголовки);
	...
	ПодготовленныйЗапрос.Заголовки["x-amz-content-sha256"] = 
		КоннекторHTTPВызовСервера.ХешированиеДанных("SHA256", ПодготовитьДанныеДляХеширования(ПодготовленныйЗапрос.HTTPЗапрос.ПолучитьТелоКакПоток()));
		
	СтруктураURL = КоннекторHTTPВызовСервера.РазобратьURLСервер(ПодготовленныйЗапрос.URL);
	
	КаноническиеЗаголовки = КоннекторHTTPВызовСервера.ПолучитьКаноническиеЗаголовкиAWS4(ПодготовленныйЗапрос.Заголовки, СтруктураURL);
	
	КаноническийПуть = СтруктураURL.Путь;
	КаноническиеПараметрыЗапроса = КоннекторHTTPВызовСервера.ПолучитьКаноническиеПараметрыЗапросаAWS4(СтруктураURL.ПараметрыЗапроса);
	...
	ЧастиСтрокиДляПодписи.Добавить(КоннекторHTTPВызовСервера.ХешированиеДанных("SHA256", КаноническийЗапрос));
	СтрокаДляПодписи = СтрСоединить(ЧастиСтрокиДляПодписи, Символы.ПС);
	
	Ключ = КоннекторHTTPВызовСервера.ПолучитьКлючПодписиAWS4(
	...
	Подпись = НРег(ПолучитьHexСтрокуИзДвоичныхДанных(КоннекторHTTPВызовСервера.ПодписатьСообщениеHMAC(Ключ, СтрокаДляПодписи)));
...
Функция ПодготовитьЗаголовокDigest(Сессия, ПодготовленныйЗапрос)
	...
	Если ЗначениеЗаполнено(СтруктураURL.ПараметрыЗапроса) Тогда
		Путь = Путь + "?" + КоннекторHTTPВызовСервера.КодироватьПараметрыЗапросаСервер(СтруктураURL.ПараметрыЗапроса);
	КонецЕсли;
	...
	HA1 = КоннекторHTTPВызовСервера.ХешированиеДанных(Алгоритм, A1);
	HA2 = КоннекторHTTPВызовСервера.ХешированиеДанных(Алгоритм, A2);
	...
	Если АлгоритмСтрокой = "MD5-SESS" Тогда
		HA1 = КоннекторHTTPВызовСервера.ХешированиеДанных(Алгоритм, СтрШаблон("%1:%2:%3", HA1, ПараметрыDigest.nonce, ЗначениеNonce)); 
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПараметрыDigest.qop) Тогда
		ЗначениеResponse = КоннекторHTTPВызовСервера.ХешированиеДанных(Алгоритм, СтрШаблон("%1:%2:%3", HA1, ПараметрыDigest.nonce, HA2));
	ИначеЕсли ПараметрыDigest.qop = "auth"
		...
		ЗначениеResponse = КоннекторHTTPВызовСервера.ХешированиеДанных(Алгоритм, СтрШаблон("%1:%2", HA1, ЗначениеNonceBit));
	Иначе
...
&НаКлиенте
Функция ПодготовитьДанныеДляХеширования(Знач Данные)
	Перем ДанныеДляХеширования, ЧтениеДанных;
	
	Если ТипЗнч(Данные) = Тип("Строка") ИЛИ ТипЗнч(Данные) = Тип("ДвоичныеДанные") Тогда
		ДанныеДляХеширования = Данные;
	ИначеЕсли ТипЗнч(Данные) = Тип("Поток") ИЛИ ТипЗнч(Данные) = Тип("ПотокВПамяти") ИЛИ ТипЗнч(Данные) = Тип("ФайловыйПоток") Тогда
		ЧтениеДанных = Новый ЧтениеДанных(Данные);
		ДанныеДляХеширования = ЧтениеДанных.Прочитать().ПолучитьДвоичныеДанные();
		ЧтениеДанных.Закрыть();
	КонецЕсли;
	
	Возврат ДанныеДляХеширования;
КонецФункции
...
Функция РаспаковатьОтвет(Ответ)
	...
	ЗаголовокHTTP = ПолучитьЗначениеЗаголовка("content-encoding", Ответ.Заголовки);
	Если ЗаголовокHTTP <> Ложь Тогда
		Если НРег(ЗаголовокHTTP) = "gzip" Тогда
...
Процедура УпаковатьЗапрос(Запрос)
	...
	ЗаголовокHTTP = ПолучитьЗначениеЗаголовка("content-encoding", Запрос.Заголовки);
	Если ЗаголовокHTTP <> Ложь Тогда
		Если НРег(ЗаголовокHTTP) = "gzip" Тогда
...
Процедура Приостановить(ДлительностьОстановкиВСекундах)
	...
	ТекущаяДата = КоннекторHTTPВызовСервера.ТекущаяУниверсальнаяДатаСервер();
