...
Процедура Тест_PostJson() Экспорт
	...
	// Вариант записи даты, функция преобразования 
	
	Json = Новый Структура;
	Json.Вставить("Дата", '20190121002400');
	Json.Вставить("Строка", Новый УникальныйИдентификатор);	
	...
	УтверждениеВерно(Результат["json"]["Дата"], "2019-01-20T19:24:00Z");
	УтверждениеВерно(Результат["json"]["Строка"], Строка(Json.Строка));	
