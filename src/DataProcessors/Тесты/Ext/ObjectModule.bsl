...
#КонецЕсли
	...
	Если ТестироватьПовторы Тогда
		...
		Тесты.Добавить("Тест_ПовторСУчетомЗаголовкаRetryAfterDuration_КонструкторПараметров");
...
Процедура Тест_ПовторСУчетомЗаголовкаRetryAfterDuration_КонструкторПараметров() Экспорт
	
	ДополнительныеПараметры = КоннекторHTTP.НовыеПараметры();
	ДополнительныеПараметры.МаксимальноеВремяПовторов = 5;
	ДополнительныеПараметры.КоэффициентЭкспоненциальнойЗадержки = 2;
	ДополнительныеПараметры.Заголовки.Вставить("X-ID", Строка(Новый УникальныйИдентификатор));
