...
&НаСервере
Функция РазобратьURLСервер(Знач URL)
...
Функция ВызватьHTTPМетод(Сессия, Метод, URL, ДополнительныеПараметры)
	...
	Пока Перенаправление < Сессия.МаксимальноеКоличествоПеренаправлений Цикл
		...
		ПодготовленныйЗапрос.URL = КодироватьСтрокуСервер(НовыйURL, "URLВКодировкеURL");
...
&НаСервере
Функция КодироватьСтрокуСервер(URL, сСпособКодированияСтроки)
...
&НаСервере
Функция РаскодироватьСтрокуСервер(URL, сСпособКодированияСтроки)
	Перем СпособКодирования;
...
Функция СформироватьНовыйURLПриПеренаправлении(Ответ)
	...
	НовыйURL = РаскодироватьСтрокуСервер(НовыйURL, "URLВКодировкеURL");
	...
	Для Каждого Параметр Из ПараметрыЗапроса Цикл
		...
		Иначе
			Для Каждого Значение Из Значения Цикл
				ЗначениеПараметра = КодироватьСтрокуСервер(Значение, "КодировкаURL");
...
&НаСервере
Функция КодироватьПараметрыЗапросаСервер(ПараметрыЗапроса)
	...
	Если ЗаголовокRetryAfter <> Ложь Тогда
		...
		Если Длительность = 0 Тогда
			...
			Если ЗначениеЗаполнено(Дата) Тогда
				Длительность = Дата - ТекущаяУниверсальнаяДатаСервер();
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Функция ИзвлечьCookies(Заголовки, URL)
	...
	ТекущееВремя = ТекущаяУниверсальнаяДатаСервер();
	Cookies = Новый Соответствие;
...
&НаСервере
Функция ПолучитьЗначениеЗаголовкаСервер(ЗаголовокHTTP, ВсеЗаголовки, Ключ = Неопределено)
...
Функция ПолучитьИдентификаторСоединения(ПараметрыСоединения)
	...
	Возврат ХешированиеДанных("MD5", СтрСоединить(ПараметрыДляРасчетаИдентификатора, ""));
...
Функция ЗаполнитьПараметрыЗапроса(Путь)
	...
	Для Каждого СтрокаКлючРавноПараметр Из СтрРазделить(Запрос, "&", Ложь) Цикл
		СтрокаКлючРавноПараметр = РаскодироватьСтрокуСервер(СтрокаКлючРавноПараметр, "URLВКодировкеURL");
...
Процедура ПодготовитьАутентификациюAWS4(ПодготовленныйЗапрос)
	...
	ЗначениеЗаголовка = ПолучитьЗначениеЗаголовкаСервер("x-amz-date", ПодготовленныйЗапрос.Заголовки);
	...
	ПодготовленныйЗапрос.Заголовки["x-amz-content-sha256"] = 
		ХешированиеДанных("SHA256", ПодготовитьДанныеДляХеширования(ПодготовленныйЗапрос.HTTPЗапрос.ПолучитьТелоКакПоток()));
		
	СтруктураURL = РазобратьURLСервер(ПодготовленныйЗапрос.URL);
	...
	ЧастиСтрокиДляПодписи.Добавить(ХешированиеДанных("SHA256", КаноническийЗапрос));
...
Функция ПодготовитьЗаголовокDigest(Сессия, ПодготовленныйЗапрос)
	...
	Если ЗначениеЗаполнено(СтруктураURL.ПараметрыЗапроса) Тогда
		Путь = Путь + "?" + КодироватьПараметрыЗапросаСервер(СтруктураURL.ПараметрыЗапроса);
	КонецЕсли;
...
&НаСервере
Функция ХешированиеДанных(Знач ИмяАлгоритма, Знач Данные)
