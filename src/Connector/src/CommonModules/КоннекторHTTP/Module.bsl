...
Функция Get(URL, ПараметрыЗапроса = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	Возврат ВызватьHTTPМетод(ТекущаяСессия, "GET", URL, Параметры); //@non-nls-1
...
Функция Options(URL, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	Возврат ВызватьHTTPМетод(ТекущаяСессия, "OPTIONS", URL, Параметры); //@non-nls-1
...
Функция Head(URL, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	Возврат ВызватьHTTPМетод(ТекущаяСессия, "HEAD", URL, Параметры); //@non-nls-1
...
Функция Post(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	Возврат ВызватьHTTPМетод(ТекущаяСессия, "POST", URL, Параметры); //@non-nls-1
...
Функция Put(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	Возврат ВызватьHTTPМетод(ТекущаяСессия, "PUT", URL, Параметры); //@non-nls-1
...
Функция Patch(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	Возврат ВызватьHTTPМетод(ТекущаяСессия, "PATCH", URL, Параметры); //@non-nls-1
...
Функция Delete(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	Возврат ВызватьHTTPМетод(ТекущаяСессия, "DELETE", URL, Параметры); //@non-nls-1
...
Функция GetJson(URL,
	...
	Возврат КакJson(ВызватьHTTPМетод(ТекущаяСессия, "GET", URL, Параметры), ПараметрыПреобразованияJSON); //@non-nls-1
...
Функция PostJson(URL, Json, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	Возврат КакJson(ВызватьHTTPМетод(ТекущаяСессия, "POST", URL, Параметры), ПараметрыПреобразованияJSON); //@non-nls-1
...
Функция PutJson(URL, Json, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	ПараметрыПреобразованияJSON =
		ВыбратьЗначение(Неопределено, Параметры, "ПараметрыПреобразованияJSON", Неопределено);
	Возврат КакJson(ВызватьHTTPМетод(ТекущаяСессия, "PUT", URL, Параметры), ПараметрыПреобразованияJSON); //@non-nls-1
...
Функция DeleteJson(URL, Json, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	ПараметрыПреобразованияJSON =
		ВыбратьЗначение(Неопределено, Параметры, "ПараметрыПреобразованияJSON", Неопределено);
	Возврат КакJson(ВызватьHTTPМетод(ТекущаяСессия, "DELETE", URL, Параметры), ПараметрыПреобразованияJSON); //@non-nls-1
...
//                               - Булево - Ложь - будет выполнен только один запрос к серверу.
//    * Json - Структура, Соответствие - данные, которые необходимо сериализовать в JSON.
...
//               *** ВозвращаемоеЗначение - Произвольный - значение, десериализованное из JSON. 
...
Функция НоваяАутентификацияDigest() Экспорт
	...
	Результат = Новый Структура;
	Результат.Вставить("Тип", "Digest"); //@non-nls-2
	Результат.Вставить("Пользователь", "");
	Результат.Вставить("Пароль", "");
...
Функция НоваяАутентификацияAWS4() Экспорт
	...
	Результат = Новый Структура;
	Результат.Вставить("Тип", "AWS4-HMAC-SHA256"); //@non-nls-2
...
//    * Заголовки - Соответствие - HTTP заголовки запроса.
//    * Аутентификация - См. НоваяАутентификацияОС - аутентификация сердствами ОС
...
Функция НовыйОтвет() Экспорт
	...
	Результат = Новый Структура;
	Результат.Вставить("Метод", "GET"); //@non-nls-2
	...
	Результат.Вставить("Кодировка", "utf-8"); //@non-nls-2
...
//   ПараметрыПреобразованияJSON - Структура - задает параметры преобразования JSON:
//     * ПрочитатьВСоответствие - Булево - Если Истина, чтение объекта JSON будет выполнено в Соответствие.
//                              - Булево   Если Ложь, объекты будут считываться в объект типа Структура.
...
Функция РазобратьURL(Знач URL) Экспорт
	...
	ДопустимыеСхемы = СтрРазделить("http,https", ","); //@non-nls-1
...
//           ** ВозвращаемоеЗначение - Произвольный - результат преобразования.
...
Функция КодыСостоянияHTTP() Экспорт
	...
	Результат = Новый Структура;
	Для Каждого Элемент Из ОписанияКодовСостоянийHTTP() Цикл
		Результат.Вставить(Элемент.Ключ, Элемент.Код);
	КонецЦикла;
...
Функция ПредставлениеКодаСостоянияHTTP(КодСостояния) Экспорт
	...
	Для Каждого Элемент Из ОписанияКодовСостоянийHTTP() Цикл
		Если Элемент.Код = КодСостояния Тогда
			ОписаниеКодаСостояния = Элемент;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПодготовитьЗапросДляРедиректа(Сессия, ПодготовленныйЗапрос, ПеренаправленныйОтвет)
	...
	Если ПеренаправленныйОтвет.КодСостояния <> КодыСостоянияHTTP.ВременноеПеренаправление_307
		...
		УдалитьЗаголовки(ПодготовленныйЗапрос.Заголовки, "content-length,content-type,transfer-encoding"); //@non-nls-1
...
Процедура ПодготовитьАутентификацию(ПодготовленныйЗапрос)
	...
	Если ЗначениеЗаполнено(ПодготовленныйЗапрос.Аутентификация) Тогда
		Если ПодготовленныйЗапрос.Аутентификация.Свойство("Тип") Тогда
			...
			Если ТипАутентификации = "digest" Тогда //@non-nls-1
				ПодготовленныйЗапрос.СобытияНаОтвет.Добавить("ОбработкаОтветаСКодом401");
			КонецЕсли;
			Если ТипАутентификации = "aws4-hmac-sha256" Тогда //@non-nls-1
...
Процедура ПодготовитьТелоЗапроса(ПодготовленныйЗапрос, Данные, Файлы, Json, ПараметрыЗаписиJSON)
	...
	Если ЗначениеЗаполнено(Файлы) Тогда
		ЗаголовокContentType = ЗакодироватьФайлы(HTTPЗапрос, Файлы, Данные);
	ИначеЕсли ЗначениеЗаполнено(Данные) Тогда
		ЗаголовокContentType = "application/x-www-form-urlencoded"; //@non-nls-1
	...
	ИначеЕсли Json <> Неопределено Тогда
		ЗаголовокContentType = "application/json"; //@non-nls-1
		СтрокаJson = ОбъектВJson(Json, ПодготовленныйЗапрос.ПараметрыПреобразованияJSON, ПараметрыЗаписиJSON);
		HTTPЗапрос.УстановитьТелоИзСтроки(СтрокаJson, КодировкаТекста.UTF8, ИспользованиеByteOrderMark.НеИспользовать);
	Иначе
		ЗаголовокContentType = Неопределено;
	КонецЕсли;
	ЗначениеЗаголовка = ЗначениеЗаголовка("content-type", ПодготовленныйЗапрос.Заголовки); //@non-nls-1
	Если ЗначениеЗаголовка = Ложь И ЗначениеЗаполнено(ЗаголовокContentType) Тогда
		ПодготовленныйЗапрос.Заголовки.Вставить("Content-Type", ЗаголовокContentType); //@non-nls-1
	КонецЕсли;
...
Функция ЗакодироватьФайлы(HTTPЗапрос, Файлы, Данные)
	...
	Возврат СтрШаблон("multipart/form-data; boundary=%1", Разделитель); //@non-nls-1
...
Функция СоздатьПолеФормы(ИсходныеПараметры)
	...
	Ключ = "Content-Disposition"; //@non-nls-1
	Если ЗначениеЗаголовка("content-disposition", Поле.Заголовки, Ключ) = Ложь Тогда //@non-nls-1
		Поле.Заголовки.Вставить("Content-Disposition", "form-data"); //@non-nls-1 //@non-nls-2
	КонецЕсли;

	Части = Новый Массив;
	Части.Добавить(Поле.Заголовки[Ключ]);
	Части.Добавить(СтрШаблон("name=""%1""", Поле.Имя)); //@non-nls-1
	Если ЗначениеЗаполнено(Поле.ИмяФайла) Тогда
		Части.Добавить(СтрШаблон("filename=""%1""", Поле.ИмяФайла)); //@non-nls-1
	КонецЕсли;

	Поле.Заголовки[Ключ] = СтрСоединить(Части, "; ");
	Поле.Заголовки["Content-Type"] = Поле.Тип; //@non-nls-1
...
Процедура ПереопределитьМетод(ПодготовленныйЗапрос, Ответ)
	...
	Если Ответ.КодСостояния = КодыСостоянияHTTP.СмотретьДругое_303 И Метод <> "HEAD" Тогда //@non-nls-1
		Метод = "GET"; //@non-nls-1
	КонецЕсли;

	// Поведение браузеров
	Если Ответ.КодСостояния = КодыСостоянияHTTP.ПеремещеноВременно_302 И Метод <> "HEAD" Тогда //@non-nls-1
		Метод = "GET"; //@non-nls-1
	КонецЕсли;
...
Функция ОтправитьЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
	...
	Пока Истина Цикл
		...
		Иначе
			ЗаголовокRetryAfter = ЗначениеЗаголовка("retry-after", Ответ.Заголовки); //@non-nls-1
		КонецЕсли;
	...
	ЗаголовокContentType = ЗначениеЗаголовка("content-type", Ответ.Заголовки); //@non-nls-1
...
Процедура УпаковатьЗапрос(Запрос)
	...
	Заголовок = ЗначениеЗаголовка("content-encoding", Запрос.Заголовки);  //@non-nls-1
	Если Заголовок <> Ложь Тогда
		Если НРег(Заголовок) = "gzip" Тогда //@non-nls-1
...
Функция РаспаковатьОтвет(Ответ)
	...
	Заголовок = ЗначениеЗаголовка("content-encoding", Ответ.Заголовки); //@non-nls-1
	Если Заголовок <> Ложь Тогда
		Если НРег(Заголовок) = "gzip" Тогда //@non-nls-1
...
Процедура ОбработкаОтветаСКодом401(Сессия, ПодготовленныйЗапрос, Настройки, Ответ)
	...
	Значение = ЗначениеЗаголовка("www-authenticate", Ответ.Заголовки); //@non-nls-1
	Если Значение <> Ложь И СтрНайти(НРег(Значение), "digest") Тогда //@non-nls-1
		Позиция = СтрНайти(НРег(Значение), "digest"); //@non-nls-1
		Значение = Сред(Значение, Позиция + СтрДлина("digest") + 1); //@non-nls-1
		Значение = СтрЗаменить(Значение, """", "");
		Значение = СтрЗаменить(Значение, Символы.ПС, "");

		ПараметрыDigest = Новый Структура("algorithm,realm,nonce,qop,opaque"); //@non-nls-1
		...
		ПодготовленныйЗапрос.Заголовки.Вставить("Authorization", ПодготовитьЗаголовокDigest(Сессия, ПодготовленныйЗапрос)); //@non-nls-1
...
Функция СформироватьНовыйURLПриПеренаправлении(Ответ)
	...
	НовыйURL = ЗначениеЗаголовка("location", Ответ.Заголовки); //@non-nls-1
...
Функция ЭтоСтандартныйПорт(СтруктураURL)
	...
	Возврат (СтруктураURL.Схема = "http" И СтруктураURL.Порт = СтандартныйПортHTTP) //@non-nls-1
		ИЛИ (СтруктураURL.Схема = "https" И СтруктураURL.Порт = СтандартныйПортHTTPS); //@non-nls-1
...
Функция НастройкиПодключения(Метод, URL, ДополнительныеПараметры)
	...
	РазрешитьПеренаправление =
		ЗначениеПоКлючу(ДополнительныеПараметры, "РазрешитьПеренаправление", ВРег(Метод) <> "HEAD"); //@non-nls-2
...
Функция Соединение(ПараметрыСоединения, Аутентификация, ДополнительныеПараметры, Сессия)
	...
	Если Не ЗначениеЗаполнено(ПараметрыСоединения.Порт) Тогда
		Если ПараметрыСоединения.Схема = "https" Тогда //@non-nls-1
...
Функция ЗаголовкиВСтроку(Заголовки)
	...
	СортированныеЗаголовки = "Content-Disposition,Content-Type,Content-Location"; //@non-nls-1
...
Функция ЕстьЗаголовокLocation(Заголовки)
	...
	Возврат ЗначениеЗаголовка("location", Заголовки) <> Ложь; //@non-nls-1
...
Функция КодировкаИзЗаголовка(Знач Заголовок)
	...
	Если ИндексРазделителя Тогда
		...
		КлючКодировки = "charset="; //@non-nls-1
	...
	Если Кодировка = Неопределено И СтрНайти(ТипСодержимого, "text") Тогда //@non-nls-1
		Кодировка = "iso-8859-1"; //@non-nls-1
	КонецЕсли;
...
Функция СформироватьЗначениеЗаголовкаHost(СтруктураURL)
	...
	Если ЗначениеЗаполнено(СтруктураURL.Порт) И НЕ ЭтоСтандартныйПорт(СтруктураURL) Тогда
		Сервер = Сервер + ":" + Формат(СтруктураURL.Порт, "ЧРГ=; ЧГ=");
	КонецЕсли;
...
Функция ПодготовитьЗаголовокDigest(Сессия, ПодготовленныйЗапрос)
	...
	Если Не ПараметрыDigest.Свойство("last_nonce") Тогда //@non-nls-1
		ПараметрыDigest.Вставить("last_nonce"); //@non-nls-1
	КонецЕсли;

	Если ПараметрыDigest.nonce = ПараметрыDigest.last_nonce Тогда
		ПараметрыDigest.nonce_count = ПараметрыDigest.nonce_count + 1;
	Иначе
		ПараметрыDigest.Вставить("nonce_count", 1); //@non-nls-1
	КонецЕсли;
	...
	Если АлгоритмСтрокой = "MD5-SESS" Тогда //@non-nls-1
	...
	ИначеЕсли ПараметрыDigest.qop = "auth" //@non-nls-1
		ИЛИ СтрРазделить(ПараметрыDigest.qop, ",", Ложь).Найти("auth") <> Неопределено Тогда //@non-nls-2
		ЗначениеNonceBit = СтрШаблон("%1:%2:%3:%4:%5", ПараметрыDigest.nonce, ЗначениеNC, ЗначениеNonce, "auth", HA2); //@non-nls-2
	...
	Если ЗначениеЗаполнено(ПараметрыDigest.opaque) Тогда
		Строки.Добавить(СтрШаблон(", opaque=""%1""", ПараметрыDigest.opaque)); //@non-nls-1
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрыDigest.algorithm) Тогда
		Строки.Добавить(СтрШаблон(", algorithm=""%1""", ПараметрыDigest.algorithm)); //@non-nls-1
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрыDigest.qop) Тогда
		Строки.Добавить(СтрШаблон(", qop=""auth"", nc=%1, cnonce=""%2""", ЗначениеNC, ЗначениеNonce)); //@non-nls-1
	КонецЕсли;

	Возврат СтрШаблон("Digest %1", СтрСоединить(Строки, "")); //@non-nls-1
...
Функция ПреобразоватьХранилищеCookiesВМассивCookies(ХранилищеCookies)
	...
	Если ТипЗнч(ХранилищеCookies) = Тип("Массив") Тогда
		Для Каждого Cookie Из ХранилищеCookies Цикл
			НовыйCookie = КонструкторCookie();
			ЗаполнитьЗначенияСвойств(НовыйCookie, Cookie);
			Cookies.Добавить(НовыйCookie);
		КонецЦикла;
...
Процедура ЗаполнитьСписокОтфильтрованнымиCookies(Cookies, СтруктураURL, Список)
	...
	Для Каждого Cookie Из Cookies Цикл
		Если Cookie.Значение.ТолькоБезопасноеСоединение = Истина И СтруктураURL.Схема <> "https" Тогда //@non-nls-1
			Продолжить;
		КонецЕсли;
...
Функция ИзвлечьCookies(Заголовки, URL)
	...
	Для Каждого ОчереднойЗаголовок Из Заголовки Цикл
		Если НРег(ОчереднойЗаголовок.Ключ) = "set-cookie" Тогда //@non-nls-1
...
Функция РаспарситьCookie(Заголовок, URL, ТекущееВремя)
	...
	Для Каждого Параметр Из СтрРазделить(Заголовок, ";", Ложь) Цикл
		...
		Если Ключ = "domain" Тогда //@non-nls-1
			Cookie.Домен = Значение;
		ИначеЕсли Ключ = "path" Тогда //@non-nls-1
			Cookie.Путь = Значение;
		ИначеЕсли Ключ = "secure" Тогда //@non-nls-1
			Cookie.ТолькоБезопасноеСоединение = Истина;
		ИначеЕсли Ключ = "max-age" Тогда //@non-nls-1
			СрокДействияMaxAge = ТекущееВремя + ЧислоИзСтроки(Значение);
		ИначеЕсли Ключ = "expires" Тогда //@non-nls-1
...
Функция КлючПодписиAWS4(СекретныйКлюч, Дата, Регион, Сервис)
	...
	КлючДата = ПодписатьСообщениеHMAC("AWS4" + СекретныйКлюч, Дата); //@non-nls-1
	КлючРегион = ПодписатьСообщениеHMAC(КлючДата, Регион);
	КлючСервис = ПодписатьСообщениеHMAC(КлючРегион, Сервис);

	Возврат ПодписатьСообщениеHMAC(КлючСервис, "aws4_request"); //@non-nls-1
...
Процедура ПодготовитьАутентификациюAWS4(ПодготовленныйЗапрос)
	...
	ЗначениеЗаголовка = ЗначениеЗаголовка("x-amz-date", ПодготовленныйЗапрос.Заголовки); //@non-nls-1
	Если ЗначениеЗаголовка <> Ложь Тогда
		ТекущееВремя = Дата(СтрЗаменить(СтрЗаменить(ЗначениеЗаголовка, "T", ""), "Z", "")); //@non-nls-1 //@non-nls-3
	Иначе
		ТекущееВремя = ТекущаяУниверсальнаяДата();
	КонецЕсли;
	ПодготовленныйЗапрос.Заголовки["x-amz-date"] = Формат(ТекущееВремя, "ДФ=yyyyMMddTHHmmssZ"); //@non-nls-1
	...
	ЧастиЗапроса.Добавить(ПодготовленныйЗапрос.Заголовки["x-amz-content-sha256"]); //@non-nls-1
	...
	ЧастиОбластиДействия.Добавить("aws4_request"); //@non-nls-1
	...
	ЧастиСтрокиДляПодписи.Добавить(ПодготовленныйЗапрос.Заголовки["x-amz-date"]); //@non-nls-1
	...
	ПодготовленныйЗапрос.Заголовки["Authorization"] = СтрШаблон( //@non-nls-1
...
Функция КаноническиеЗаголовкиAWS4(Заголовки, СтруктураURL)
	...
	Для Каждого ОчереднойЗаголовок Из Заголовки Цикл
		...
		Если ЗаголовкиПоУмолчанию.Исключения.Найти(Заголовок) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ЗаголовокHostЕстьВЗапросе = Макс(ЗаголовокHostЕстьВЗапросе, Заголовок = "host"); //@non-nls-1
	...
	Если Не ЗаголовокHostЕстьВЗапросе Тогда
		Список.Добавить("host", СформироватьЗначениеЗаголовкаHost(СтруктураURL)); //@non-nls-1
	КонецЕсли;
...
Функция ЗаголовкиПоУмолчаниюAWS4()
	...
	Заголовки.Вставить("Равно", СтрРазделить("host,content-type,date", ",")); //@non-nls-2
	Заголовки.Вставить("НачинаетсяС", СтрРазделить("x-amz-", ",")); //@non-nls-2
	Заголовки.Вставить("Исключения", СтрРазделить("x-amz-client-context", ",")); //@non-nls-2
...
Функция ZipLFH()
	...
	Буфер.Записать(30, ПолучитьБуферДвоичныхДанныхИзСтроки("data", "ascii", Ложь)); //@non-nls-1 //@non-nls-2
...
Функция ZipCDH(CRC32, РазмерСжатыхДанных, РазмерНесжатыхДанных)
	...
	Буфер.Записать(46, ПолучитьБуферДвоичныхДанныхИзСтроки("data", "ascii", Ложь)); //@non-nls-1 //@non-nls-2
...
#Если МобильноеПриложениеСервер Тогда
	ВызватьИсключение(НСтр("ru = 'Работа с Zip-файлами в мобильной платформе не поддерживается'"));
#Иначе
	ВременныйФайл = ПолучитьИмяВременногоФайла(".bin"); //@non-nls-1
...
#Если МобильноеПриложениеСервер Тогда
	Заголовки.Вставить("Accept-Encoding", "identity"); //@non-nls-1 //@non-nls-2
#Иначе
	Заголовки.Вставить("Accept-Encoding", "gzip"); //@non-nls-1 //@non-nls-2
#КонецЕсли
	Заголовки.Вставить("Accept", "*/*"); //@non-nls-1
	Заголовки.Вставить("Connection", "keep-alive"); //@non-nls-1 //@non-nls-2
...
Функция ОписанияКодовСостоянийHTTP()
	...
	Коды = Новый Массив;
	Коды.Добавить(НовыйКодHTTP(100, "Продолжай_100", "Continue")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(101, "ПереключениеПротокола_101", "Switching Protocols")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(102, "ИдетОбработка_102", "Processing")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(103, "РанняяМетаинформация_103", "Early Hints")); //@non-nls-2

	Коды.Добавить(НовыйКодHTTP(200, "ОК_200", "OK")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(201, "Создано_201", "Created")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(202, "Принято_202", "Accepted")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(203, "ИнформацияНеАвторитетна_203", "Non-Authoritative Information")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(204, "НетСодержимого_204", "No Content")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(205, "СброситьСодержимое_205", "Reset Content")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(206, "ЧастичноеСодержимое_206", "Partial Content")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(207, "Многостатусный_207", "Multi-Status")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(208, "УжеСообщалось_208", "Already Reported")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(226, "ИспользованоIM_226", "IM Used")); //@non-nls-2

	Коды.Добавить(НовыйКодHTTP(300, "МножествоВыборов_300", "Multiple Choices")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(301, "ПеремещеноНавсегда_301", "Moved Permanently")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(302, "ПеремещеноВременно_302", "Moved Temporarily")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(303, "СмотретьДругое_303", "See Other")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(304, "НеИзменялось_304", "Not Modified")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(305, "ИспользоватьПрокси_305", "Use Proxy")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(307, "ВременноеПеренаправление_307", "Temporary Redirect")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(308, "ПостоянноеПеренаправление_308", "Permanent Redirect")); //@non-nls-2

	Коды.Добавить(НовыйКодHTTP(400, "НеверныйЗапрос_400", "Bad Request")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(401, "НеАвторизован_401", "Unauthorized")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(402, "НеобходимаОплата_402", "Payment Required")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(403, "Запрещено_403", "Forbidden")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(404, "НеНайдено_404", "Not Found")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(405, "МетодНеПоддерживается_405", "Method Not Allowed")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(406, "Неприемлемо_406", "Not Acceptable")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(407, "НеобходимаАутентификацияПрокси_407", "Proxy Authentication Required")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(408, "ИстеклоВремяОжидания_408", "Request Timeout")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(409, "Конфликт_409", "Conflict")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(410, "Удален_410", "Gone")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(411, "НеобходимаДлина_411", "Length Required")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(412, "УсловиеЛожно_412", "Precondition Failed")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(413, "ПолезнаяНагрузкаСлишкомВелика_413", "Payload Too Large")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(414, "СлишкомДлинныйURI_414", "URI Too Long")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(415, "НеподдерживаемыйТипДанных_415", "Unsupported Media Type")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(416, "ДиапазонНеДостижим_416", "Range Not Satisfiable")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(417, "ОжиданиеНеУдалось_417", "Expectation Failed")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(419, "ОшибкаПроверкиCSRF_419", "Authentication Timeout")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(421, "НеправильноНаправленныйЗапрос_421", "Misdirected Request")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(422, "НеобрабатываемыйЭкземпляр_422", "Unprocessable Entity")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(423, "Заблокировано_423", "Locked")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(424, "НевыполненнаяЗависимость_424", "Failed Dependency")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(425, "СлишкомРано_425", "Too Early")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(426, "НеобходимоОбновление_426", "Upgrade Required")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(428, "НеобходимоПредусловие_428", "Precondition Required")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(429, "СлишкомМногоЗапросов_429", "Too Many Requests")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(431, "ПоляЗаголовкаЗапросаСлишкомБольшие_431", "Request Header Fields Too Large")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(449, "ПовторитьС_449", "Retry With")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(451, "НедоступноПоЮридическимПричинам_451", "Unavailable For Legal Reasons")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(499, "КлиентЗакрылСоединение_499", "Client Closed Request")); //@non-nls-2

	Коды.Добавить(НовыйКодHTTP(500, "ВнутренняяОшибкаСервера_500", "Internal Server Error")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(501, "НеРеализовано_501", "Not Implemented")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(502, "ОшибочныйШлюз_502", "Bad Gateway")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(503, "СервисНедоступен_503", "Service Unavailable")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(504, "ШлюзНеОтвечает_504", "Gateway Timeout")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(505, "ВерсияHTTPНеПоддерживается_505", "HTTP Version Not Supported")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(506, "ВариантТожеПроводитСогласование_506", "Variant Also Negotiates")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(507, "ПереполнениеХранилища_507", "Insufficient Storage")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(508, "ОбнаруженоБесконечноеПеренаправление_508", "Loop Detected")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(509, "ИсчерпанаПропускнаяШиринаКанала_509", "Bandwidth Limit Exceeded")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(510, "НеРасширено_510", "Not Extended")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(511, "ТребуетсяСетеваяАутентификация_511", "Network Authentication Required")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(520, "НеизвестнаяОшибка_520", "Unknown Error")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(521, "ВебСерверНеРаботает_521", "Web Server Is Down")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(522, "СоединениеНеОтвечает_522", "Connection Timed Out")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(523, "ИсточникНедоступен_523", "Origin Is Unreachable")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(524, "ВремяОжиданияИстекло_524", "A Timeout Occurred")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(525, "КвитированиеSSНеУдалось_525", "SSL Handshake Failed")); //@non-nls-2
	Коды.Добавить(НовыйКодHTTP(526, "НедействительныйСертификатSSL_526", "Invalid SSL Certificate")); //@non-nls-2
...
Функция ОпределитьХешФункцию(Знач Алгоритм)
	...
	Если Не ЗначениеЗаполнено(Алгоритм) ИЛИ Алгоритм = "MD5" ИЛИ Алгоритм = "MD5-SESS" Тогда //@non-nls-1  //@non-nls-2
		АлгоритмХеширования = ХешФункция.MD5;
	ИначеЕсли Алгоритм = "SHA" Тогда //@non-nls-1
		АлгоритмХеширования = ХешФункция.SHA1;
	ИначеЕсли Алгоритм = "SHA-256" Тогда //@non-nls-1
...
Процедура Приостановить(ДлительностьОстановкиВСекундах)
	...
	Пока ТекущаяДата < ЖдатьДо Цикл
		...
		Попытка
			...
			Соединение.Получить(Новый HTTPЗапрос("/does_not_matter")); //@non-nls-1
		Исключение
...
Функция ДатаИзСтрокиRFC7231(Знач Строка) Экспорт
	...
	Месяцы = СтрРазделить("Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec", ","); //@non-nls-1
