...
Процедура Тест_РазобратьURLСЗакодированнымURLВПараметре() Экспорт
	...
	УтверждениеВерно(СтруктураURL.ПараметрыЗапроса["url"], "http://www.kuku.ru/?s=1&b=2"); //@non-nls-1
...
Процедура Тест_ПередачаПараметровВСтрокуЗапроса() Экспорт
	...
	УтверждениеВерно(Результат["url"], "https://httpbin.org/anything/params?name=Иванов&name=Петров&salary=100000&time=01%3A47"); //@non-nls-1
	УтверждениеВерно(Результат["args"]["salary"], "100000"); //@non-nls-1 //@non-nls-2
	УтверждениеВерно(СтрСоединить(Результат["args"]["name"], ","), "Иванов,Петров"); //@non-nls-1 //@non-nls-2 //@non-nls-3
...
Процедура Тест_ПередачаПроизвольныхЗаголовков() Экспорт
	...
	Заголовки.Вставить("X-My-Header", "Hello"); //@non-nls-1
...
Процедура Тест_ОтправитьGZip() Экспорт
	...
	УтверждениеВерно(Результат["headers"]["Content-Encoding"], "gzip"); //@non-nls-1 //@non-nls-2 //@non-nls-31
...
Процедура Тест_BasicAuth() Экспорт
	...
	УтверждениеВерно(Результат["authenticated"], Истина); //@non-nls-1
	УтверждениеВерно(Результат["user"], "user"); //@non-nls-1
	...
	Результат = КоннекторHTTP.GetJson(
		...
		Неопределено,
		Новый Структура("Аутентификация", Аутентификация));
	УтверждениеВерно(Результат["authenticated"], Истина); //@non-nls-2
	УтверждениеВерно(Результат["user"], "user"); //@non-nls-1 //@non-nls-2
...
Процедура Тест_ПроверкаВосстановленияНеподдерживаемыхТиповЗначений() Экспорт
	...
	ДвоичныеДанные = ПолучитьДвоичныеДанныеИзСтроки("test", "utf-8", Ложь); //@non-nls-1
...
Процедура Тест_PutJson() Экспорт
	...
	УтверждениеВерно(Результат["url"], "https://httpbin.org/put"); //@non-nls-1
...
Процедура Тест_ОтправитьCookies() Экспорт
	...
	Cookies.Добавить(Новый Структура("Наименование,Значение", "k1", Строка(Новый УникальныйИдентификатор))); //@non-nls-1 //@non-nls-2
	Cookies.Добавить(Новый Структура("Наименование,Значение", "k2", Строка(Новый УникальныйИдентификатор))); //@non-nls-1 //@non-nls-2
...
Процедура Тест_POST_MultipartFormData_ТолькоФайл() Экспорт
	...
	Файлы.Вставить("Имя", "f1"); //@non-nls-1 //@non-nls-2
	Файлы.Вставить("ИмяФайла", "file1.txt"); //@non-nls-1 //@non-nls-2
	Файлы.Вставить("Данные", Base64Значение("0J/RgNC40LLQtdGCLCDQnNC40YA=")); //@non-nls-1 //@non-nls-2
	Файлы.Вставить("Тип", "text/plain"); //@non-nls-1 //@non-nls-2
...
Процедура Тест_POST_MultipartFormData_ФайлыИПоляФормы() Экспорт
	...
	Файлы = Новый Массив;
	Файлы.Добавить(Новый Структура("Имя,Данные,ИмяФайла", "f1", Base64Значение("ZmlsZTE="), "file1.txt")); //@non-nls-1 //@non-nls-2 //@non-nls-3 //@non-nls-4
	Файлы.Добавить(Новый Структура("Имя,Данные,ИмяФайла", "f2", Base64Значение("ZmlsZTI="), "file2.txt")); //@non-nls-1 //@non-nls-2 //@non-nls-3 //@non-nls-4
