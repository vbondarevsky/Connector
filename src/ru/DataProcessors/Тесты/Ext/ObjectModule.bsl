...
Процедура Тест_BasicAuth() Экспорт
	...
	Результат = КоннекторHTTP.НовыйПостроительHTTPЗапроса(
		"https://httpbin.org/basic-auth/user/pass", "GET")
		.УстановитьАутентификациюBasic("user", "pass")
		.ВыполнитьЗапрос()
		.КакJson();
	УтверждениеВерно(Результат["authenticated"], Истина);
	УтверждениеВерно(Результат["user"], "user"); 
...
Процедура Тест_DigestAuth() Экспорт
	...
	Результат = КоннекторHTTP.НовыйПостроительHTTPЗапроса(
		"https://httpbin.org/basic-auth/user/pass", "GET")
		.УстановитьАутентификациюDigest("user", "pass")
		.ВыполнитьЗапрос();
	УтверждениеВерно(Результат.КодСостояния, 200);
...
Процедура Тест_GetJson() Экспорт
	...
	Результат = КоннекторHTTP.НовыйПостроительHTTPЗапроса("https://httpbin.org/get", "GET")
		.ВыполнитьЗапрос()
		.КакJson();
...
Процедура Тест_Options() Экспорт
	...
	Ответ = КоннекторHTTP.НовыйПостроительHTTPЗапроса("http://httpbin.org/anything", "OPTIONS")
		.ВыполнитьЗапрос();
...
Процедура Тест_Head() Экспорт
	...
	УтверждениеВерно(Ответ.КодСостояния, 200);
		...
		Ответ = КоннекторHTTP.НовыйПостроительHTTPЗапроса("http://httpbin.org/anything", "HEAD")
		.ВыполнитьЗапрос();
...
Процедура Тест_Delete() Экспорт
	...
	Ответ = КоннекторHTTP.НовыйПостроительHTTPЗапроса("http://httpbin.org/anything", "DELETE")
		.ВыполнитьЗапрос();
...
Процедура Тест_Patch() Экспорт
	...
	УтверждениеВерно(Ответ.КодСостояния, 200);
		...
		Ответ = КоннекторHTTP.НовыйПостроительHTTPЗапроса("http://httpbin.org/anything", "PATCH")
		.ВыполнитьЗапрос();
...
Процедура Тест_POST_MultipartFormData_ТолькоФайл() Экспорт
	...
	УтверждениеВерно(Результат["files"]["f1"], "Привет, Мир"); 
		...
		.ДобавитьФайл("f1", "file1.txt", Base64Значение("0J/RgNC40LLQtdGCLCDQnNC40YA="))
		.ВыполнитьЗапрос()
		.КакJson();
...
Процедура Тест_POST_MultipartFormData_ФайлыИПоляФормы() Экспорт
	...
	Результат = КоннекторHTTP
		...
		.УстановитьДанные(Новый Структура("field1,field2", "value1", "Значение2"))
		.ВыполнитьЗапрос()
		.КакJson();
...
Процедура Тест_ОтправкаXml() Экспорт
	...
	Ответ = КоннекторHTTP
		.НовыйПостроительHTTPЗапроса("https://www.cbr.ru/DailyInfoWebServ/DailyInfo.asmx", "POST")
		.УстановитьЗаголовок("Content-Type", "text/xml; charset=utf-8")
		.УстановитьЗаголовок("SOAPAction", "http://web.cbr.ru/GetCursOnDate")
		.УстановитьДанные(XML)
		.ВыполнитьЗапрос();
