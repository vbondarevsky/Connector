...
Процедура Тест_BasicAuth() Экспорт
	...
	.НовыйПостроительHTTPЗапроса("http://httpbin.org/basic-auth/foo/bar", "GET")
	.УстановитьАутентификациюBasic("foo", "bar")
	.ВыполнитьЗапрос()
	.КакJson();
	
	УтверждениеВерно(Результат["authenticated"], Истина);
	УтверждениеВерно(Результат["user"], "foo");
...
Процедура Тест_DigestAuth() Экспорт
	...
	Ответ = КоннекторHTTP
		.НовыйПостроительHTTPЗапроса("https://jigsaw.w3.org/HTTP/Digest/", "GET")
		.УстановитьАутентификациюDigest("guest", "guest")
		.ВыполнитьЗапрос()
	;
