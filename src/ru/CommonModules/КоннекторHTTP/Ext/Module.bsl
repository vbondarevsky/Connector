...
// Конструктор построителя HTTP запроса
//
// Параметры:
//
//   URL - Строка - URL ресурса, к которому будет отправлен запрос.
...
//
// Возвращаемое значение:
//  ОбработкаОбъект.ПостроительHTTPЗапроса - Объект построителя запросов
//
Функция НовыйПостроительHTTPЗапроса(URL = "", Метод = "") Экспорт
	
	ПараметрыПостроителя = НовыйПараметрыПостроителяHTTPЗапроса();
	ПараметрыПостроителя.URL = URL;
	ПараметрыПостроителя.Метод = Метод;
	
	ПостроительHTTPЗапроса = Обработки.ПостроительHTTPЗапроса.Создать();
	ПостроительHTTPЗапроса.Инициализировать(ПараметрыПостроителя);
	
	Возврат ПостроительHTTPЗапроса;
...
Функция НовыйПараметрыПостроителяHTTPЗапроса()
	
	ПараметрыПостроителяHTTPЗапроса = Новый Структура();
	ПараметрыПостроителяHTTPЗапроса.Вставить("URL", "");
	ПараметрыПостроителяHTTPЗапроса.Вставить("Метод", "");
	
	Возврат ПараметрыПостроителяHTTPЗапроса;
КонецФункции
#КонецОбласти
